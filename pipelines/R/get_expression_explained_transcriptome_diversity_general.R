...
cor_pca_transcriptome_diverisity <- function(transcriptome_diversity, pca_loadings) {
    ...
    cors <- merged %>%
        ...
        summarise(pearson_cor = cor_test(transcriptome_diversity, feature_value, val='estimate'),
                  ...
                  spearman_cor = cor_test(transcriptome_diversity, feature_value, val='estimate', method='spearman'),
                  pvalue_spearman = cor_test(transcriptome_diversity, feature_value, val='p.value', method='spearman'),
                  signed_pvalue = sign(pearson_cor) * -log10(pvalue)) %>%
        ungroup() %>%
        filter(!is.na(pearson_cor)) %>%
        mutate(pvalue_bonf = p.adjust(pvalue), pvalue_spearman_bonf=p.adjust(pvalue_bonf)) %>%
